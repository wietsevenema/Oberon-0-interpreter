...
public class StatementEvaluatorTest {
	...
	public void testAssignment() throws VariableAlreadyDeclaredException,
		...
		AssignmentStatement as1 = new AssignmentStatement(new Identifier("a"),
				new IntegerConstant(2));

		AssignmentStatement as2 = new AssignmentStatement(new Identifier("b"),
		...
		assertEquals(((IntegerValue) symbolTable.lookupValue("a")).getValue(),
				new Integer(2));
		assertEquals(((BooleanValue) symbolTable.lookupValue("b")).getValue(),
				new Boolean(true));
	}

	@Test
	public void testSecondAssignment() throws ValueUndefinedException,
			VariableNotDeclaredException, VariableAlreadyDeclaredException {
		symbolTable.declareValue("a", new IntegerValue(null));
		AssignmentStatement first = new AssignmentStatement(
				new Identifier("a"), new IntegerConstant(2));

		AssignmentStatement second = new AssignmentStatement(
				new Identifier("a"), new IntegerConstant(3));
	...
	public void testCallNotExistingProcedureFails() throws Throwable {
		Statement call = new ProcedureCallStatement(new Identifier(
				"idonotexist"), new ArrayList<Node>());
	...
	@Test
	public void testWhileStatement() throws VariableAlreadyDeclaredException,
			ValueUndefinedException, VariableNotDeclaredException {

		Expression condition = new EqualityExpression(new Identifier("count"),
				new IntegerConstant(5), "<");

		Statement stat1 = new AssignmentStatement(new Identifier("count"),
				new AdditiveExpression(new Identifier("count"),
						new IntegerConstant(1), "+"));

		Statement stat2 = new AssignmentStatement(new Identifier("touch"),
				new BooleanConstant(true));
		...
		assertEquals(new Integer(5),
				((IntegerValue) symbolTable.lookupValue("count")).getValue());
		assertEquals(new Boolean(true),
				((BooleanValue) symbolTable.lookupValue("touch")).getValue());

	}

	@Test
	public void testWhileStatement2() throws InvalidInputException,
			ParseException, IOException, ValueUndefinedException,
			VariableNotDeclaredException {
		String whileprog = "MODULE While;	" + "	VAR t1: INTEGER; t2 : BOOLEAN;"
				+ "BEGIN" + " t1 := 0;" + " WHILE t1 <= 5" + " DO"
				+ "	t1  := t1 + 1;" + "	t2 := TRUE" + " END " + "END While.";

		ModuleEvaluator me = new ModuleEvaluator(symbolTable);
		me.dispatch(Util.parseModuleString(whileprog));

		assertEquals(new Boolean(true),
				((BooleanValue) symbolTable.lookupValue("t2")).getValue());
		assertEquals(new Integer(6),
				((IntegerValue) symbolTable.lookupValue("t1")).getValue());
	}

	@Test
	public void testIfStatement() throws IOException, InvalidInputException,
			ParseException {
		Module result = (Module) Util.parseModuleFile(Util
				.getAbsFilename("oberon/ifstatement.o0"));
	...
	@Test
	public void testAssignStatement() throws IOException,
			InvalidInputException, ParseException {
		Module result = (Module) Util.parseModuleFile(Util
				.getAbsFilename("oberon/statements/assignment.o0"));
		Environment env = new Environment(System.in, System.out);
		BuiltIns.inject(env);
		env.runModule(result);
	}
