...
public class TransformGenerics extends Visitor {
	...
	public Node visit(Node n) throws TransformedException {
		if (n instanceof Expression) {
			...
			throw new TransformedException("Expected expression but got "
					+ n.getName());
		}
	}
	...
	public Selector visitArraySelector(GNode n) {
		 ...
		 * <Record> Selector void:".":Symbol Identifier / <Base>
		 * yyValue:Identifier
		 */
	...
	/**
	 * Some magic to transform a generic expression node to it's static typed
	 * equivalent.
	 * 
	 * @param n
	 * @return
	 ...
	 */
	private BinaryExpression createBinaryExpression(GNode n)
			throws ExpressionNotFoundException {
		Expression left, right;
		String token;
		left = (Expression) dispatch(n.getNode(0));
		...
		try {
			...
			Class<BinaryExpression> expression = (Class<BinaryExpression>) Class
					.forName("eu.wietsevenema.lang.oberon.ast.expressions."
							+ n.getName());
			Class<?>[] args = {
					Class.forName("eu.wietsevenema.lang.oberon.ast.expressions.Expression"),
					Class.forName("eu.wietsevenema.lang.oberon.ast.expressions.Expression"),
					Class.forName("java.lang.String") };
			BinaryExpression result = (BinaryExpression) expression
					.getConstructor(args).newInstance(left, right, token);
			return result;
		} catch (ClassNotFoundException e) {
			throw new ExpressionNotFoundException("Binary expression '"
					+ n.getName() + "' has no corresponding class definition.");
	...
	public BinaryExpression visitEqualityExpression(GNode n)
			throws ExpressionNotFoundException {
		return createBinaryExpression(n);
	}

	public BinaryExpression visitAdditiveExpression(GNode n)
			throws ExpressionNotFoundException {
		return createBinaryExpression(n);
	}

	public BinaryExpression visitMultiplicativeExpression(GNode n)
			throws ExpressionNotFoundException {
		return createBinaryExpression(n);
	}

	public BinaryExpression visitLogicalConjunctiveExpression(GNode n)
			throws ExpressionNotFoundException {
		return createBinaryExpression(n);
	}

	public BinaryExpression visitLogicalDisjunctiveExpression(GNode n)
			throws ExpressionNotFoundException {
		return createBinaryExpression(n);
	}
